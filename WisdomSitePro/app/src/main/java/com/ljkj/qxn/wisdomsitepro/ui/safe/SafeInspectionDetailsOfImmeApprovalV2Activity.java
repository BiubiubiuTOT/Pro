 ...
 * 修改备注：
 */
public class SafeInspectionDetailsOfImmeApprovalV2Activity extends SafeInspectionDetailV2Activity implements InspectRectifyContract.View {
    ...
    protected void doNext() {
        ...
        String id = safePatrolDetailInfo.getReform().isEmpty() ? "" :
                safePatrolDetailInfo.getReform().get(safePatrolDetailInfo.getReform().size() - 1).getId();
        presenter.checkRectifyDetail(id, status, reason, safePatrolDetailInfo.getCheck().getId());
    }
